...
for p, size in sets:
    if not p.exists():
        print(f"resizeing to {size} into {p}")
        pararrel(partial(resize_one, path=p, size=size), il.items)

bs, size=32, 128
arch=models.resnet34

src=ImageImageList.from_folder(path_lr).split_y_rand_pct(0.1, seed = 42)
...
class FeatureLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        self.feat_losses = [base_loss(input, target)]
        self.feat_losses += [base_loss(f_in, f_out)*w
                            for f_in, f_out, w in zip(in_feat, out_feat, self_wgts)]
...
def do_fit(save_name, lrs=slice(lr), pct_start=0.9):
    learn_fit_one_cycle(10,lrs,pct_start=pct_start)
    learn.save(save_name)
    learn.show_results(row=1,imgsize=5)
